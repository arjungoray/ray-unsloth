"""Run three concurrent Qwen3.5 4B LoRA RL jobs on one shared A100.

Both tenants train with the same low-level RL primitive shape:
...
SETTINGS_KEY = "qwen3_5_4b_multitenant_rl"
...
async def train_one_tenant(
    *,
    ...
    run_name = f"{settings.get('wandb', {}).get('name', 'qwen3.5-4b-1xa100-multitenant-rl')}/{spec.name}"
...
async def train(args: argparse.Namespace) -> None:
    ...
    specs = tenant_specs()
    group_name = str(settings.get("wandb", {}).get("name", "qwen3.5-4b-1xa100-multitenant-rl"))
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/qwen3_5_4b_1x_a100_multitenant_rl.yaml")
