"""Run two concurrent Qwen3.5 4B LoRA SFT jobs on one shared A100.
...
while both trainer sessions route to one Modal A100 container pool.
...
SETTINGS_KEY = "qwen3_5_4b_multitenant_sft"
...
async def train_one_tenant(
    *,
    ...
    run_name = f"{settings.get('wandb', {}).get('name', 'qwen3.5-4b-1xa100-multitenant-sft')}/{spec.name}"
...
async def train(args: argparse.Namespace) -> None:
    ...
    group_name = str(settings.get("wandb", {}).get("name", "qwen3.5-4b-1xa100-multitenant-sft"))
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/qwen3_5_4b_1x_a100_multitenant.yaml")
